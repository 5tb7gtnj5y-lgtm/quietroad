  ...
  return <div className="map-frame">
    ...
    <div className="map-canvas" ref={frame}>
      <svg viewBox={'0 0 ' + width + ' ' + height} role="img" aria-label={plan.to ? 'Street map of ' + plan.from.label + (plan.stop ? ' via ' + plan.stop.name : '') + ' to ' + plan.to.label + ', with nearby Greggs' : 'Street map around ' + plan.from.label} onPointerDown={event => {
        ...
        <circle cx={start.x} cy={start.y} r="13" fill="#20a6df" stroke="#07375a" strokeWidth="4" /><circle cx={start.x} cy={start.y} r="4" fill="#173548" />
      </svg>
...
function Profile({ plan }: { plan: Plan }) {
  ...
  return <section className="surface profile-panel" aria-labelledby="profile-heading">
    <div className="section-head"><div><p className="eyebrow">{plan.profileSource === 'gb-2025' ? 'National day-of-week pattern' : 'Measured road counts'}</p><h2 id="profile-heading">{plan.profileSource === 'gb-2025' ? 'Typical traffic by hour across Great Britain' : 'How the road changes through the day'}</h2></div><span className="small-chip">{new Date(plan.requestedDate + 'T12:00:00Z').toLocaleDateString('en-GB', { weekday: 'long', timeZone: 'UTC' })} · 07:00–19:00</span></div>
    {plan.hourly.length ? <>
      <p className="subtle">{plan.profileSource === 'gb-2025' ? 'These bars use the 2025 DfT national pattern for this day of the week, across all roads. It is not a count of your chosen road.' : 'Bar height compares each measured hour with the busiest measured hour. ' + (plan.samples.length === 1 ? 'Vehicle counts are from one nearby count point.' : 'Several count points are combined as a relative pattern.')}</p>
      <div className="bar-chart" role="img" aria-label={'Hourly relative traffic: ' + plan.hourly.map(h => hourLabel(h.hour) + ' ' + h.index + ' percent of peak').join(', ')}>
        {plan.hourly.map(h => <div className="bar-column" key={h.hour} title={hourLabel(h.hour) + ': ' + h.index + '% of daytime peak'}>
      ...
      <div className="profile-key"><span><i className="key-best" />Four quieter hours</span><span><i className="key-other" />Other daytime hours</span></div>
    ...
    {plan.profileSource === 'local-weekday' && plan.samples.length > 0 && <details className="data-details"><summary>Where these figures came from</summary><div>{plan.samples.map(s => <p key={s.countPointId}><strong>{s.road}</strong> · DfT count point {s.countPointId} · {s.year} · {s.distanceKm.toFixed(1)} km from route/spot · {s.hours.length} measured hours</p>)}<p>These are sample-day counts, not a live traffic forecast. Overnight and special event patterns are not covered.</p></div></details>}{plan.profileSource === 'gb-2025' && <details className="data-details"><summary>Where these figures came from</summary><div><p>Department for Transport, table TRA0307, 2025: average traffic distribution by hour and day of the week across Great Britain. The app compares 07:00–19:00 hours with the busiest hour in that range.</p><p>{plan.samples.length ? 'Nearby roadside counts were found, but were collected on weekdays and are not used for this chart.' : 'No local roadside count was available for this route.'} This national profile is a guide, not a measured count or a travel-time forecast for this road.</p><p><a href="https://www.gov.uk/government/statistical-data-sets/road-traffic-estimates-tra#annual-daily-traffic-flow-and-distribution-tra03" target="_blank" rel="noreferrer">View the DfT table TRA0307</a></p></div></details>}
  </section>;
}
...
export default function Home() {
  ...
  const [date, setDate] = useState(todayInUK);
  ...
  return <main className={night ? "site-shell theme-night" : "site-shell"}>
    <header className="topbar"><div className="brand"><span className="brand-symbol"><Navigation size={18} strokeWidth={2.5} /></span><span>quietroad<span className="brand-period">.</span></span></div><div className="topbar-right"><span>UK traffic & stop planner</span><span className="topbar-badge">BETA</span><button className="theme-toggle" type="button" aria-label={night ? "Switch to day mode" : "Switch to night mode"} aria-pressed={night} onClick={toggleTheme}>{night ? <><Sun size={16} /> Day</> : <><Moon size={16} /> Night</>}</button></div></header>
    <div className="main-grid">
      <aside className="planner" aria-label="Plan a road journey">
        <div className="planner-kicker"><p className="eyebrow light">CHOOSE WHEN TO GO</p><span className="pastry-pair" role="img" aria-label="A small steak bake and sausage roll"><svg viewBox="0 0 58 24" aria-hidden="true"><defs><linearGradient id="pastryGold" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ffd98a" /><stop offset="1" stopColor="#d88a28" /></linearGradient></defs><rect x="1.5" y="3" width="25" height="18" rx="3.5" fill="url(#pastryGold)" stroke="#985716" strokeWidth="1.2" /><path d="M6 6.5l5.5 11M11.5 5.5l5.5 13M17 5.5l5 11" stroke="#fff0bc" strokeWidth="1.4" strokeLinecap="round" opacity=".9" /><rect x="31" y="6" width="25.5" height="13" rx="6.5" fill="url(#pastryGold)" stroke="#985716" strokeWidth="1.2" /><path d="M36 8.5l3 8M42 8l3 8M48 8l3 7.5" stroke="#fff0bc" strokeWidth="1.3" strokeLinecap="round" opacity=".9" /></svg></span></div><h1>Find the <em>quieter</em> way.</h1>
        ...
        <Tabs value={mode} onValueChange={v => { setMode(v as 'journey' | 'area'); setPlan(null); setSelectedStop(null); setError(''); }} className="mode-tabs">
          ...
          <form onSubmit={search}>
            ...
            <label className="check-row" htmlFor="greggs"><Checkbox id="greggs" checked={showGreggs} onCheckedChange={v => setShowGreggs(v === true)} /><span><strong>Show Greggs stops</strong><small>Add a bakery stop along the way</small></span></label>
      ...
      <div className="results" aria-live="polite">
        {!plan && !loading && !error && <div className="welcome"><div className="welcome-copy"><p className="eyebrow">READY WHEN YOU ARE</p><h2>Your best time to leave starts here.</h2><p>Pick a quieter hour, see live speed samples and add a Greggs stop if you fancy. Choose <strong>Find quieter times</strong> to begin.</p><span className="example-pill"><MapPin size={16} /> Blyth to Newcastle</span></div></div>}
        ...
        {plan && <>
          ...
          <div className="result-top">
            <section className="surface times-panel" aria-labelledby="times-heading"><div className="section-head"><div><p className="eyebrow">FOUR TIME WINDOWS</p><h3 id="times-heading">Quieter times to go</h3></div><span className="time-badge">{new Date(plan.requestedDate + 'T12:00:00Z').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', timeZone: 'UTC' })}</span></div>
              {plan.windows.length ? <><div className="window-list">{plan.windows.map((w, i) => <div className={'window-card ' + (i === 0 ? 'best' : '')} key={w.start}><span className="window-rank">{String(i + 1).padStart(2, '0')}</span><div><strong>{hourLabel(w.start)}–{hourLabel(w.end)}</strong><small>{i === 0 ? (plan.profileSource === 'gb-2025' ? 'Quietest typical hour' : 'Quietest measured hour') : 'Among the four quieter hours'}</small></div><div className="window-meter"><b>{w.index}%</b><span>of peak</span></div></div>)}</div><p className="tiny-note">{plan.profileSource === 'gb-2025' ? 'DfT 2025 national pattern for this day of the week, not measurements of this road. Lower % means lower typical flow relative to the daytime peak; it is not a journey-time prediction.' : 'Based on historical roadside sample counts. Lower % means fewer vehicles than the busiest measured hour, not a predicted journey time.'}</p></> : <div className="window-empty"><Info size={24} /><p>Traffic data is unavailable for this search. Try again later.</p></div>}
            </section>
            <div className="side-stack"><MapSketch key={plan.from.label + '|' + (plan.to?.label || '')} plan={plan} /><div className="live-card"><div><span className={'status-light ' + (plan.liveStatus === 'available' ? 'active' : '')} /><strong>{plan.liveStatus === 'available' ? 'Live speed samples' : 'Live traffic'}</strong></div>{plan.liveStatus === 'available' ? <p>{liveAvg} mph now vs {freeAvg} mph in free flow, from {plan.live.length} sampled road {plan.live.length === 1 ? 'segment' : 'segments'}.{plan.live.some(s => s.closure) ? ' A sampled segment is reported closed.' : ''}</p> : <p>{plan.liveStatus === 'unavailable' ? 'Live traffic unavailable right now. Try again later.' : 'Live traffic is not configured yet.'}{usualNow ? ' At this hour the historical count is ' + usualNow.index + '% of the ' + (plan.profileSource === 'gb-2025' ? 'national daytime peak for this day.' : 'local measured peak.') : ''}</p>}</div></div>
          </div>
          <Profile plan={plan} />
          ...
          <footer className="result-sources">Traffic patterns: <a href="https://www.gov.uk/government/statistical-data-sets/road-traffic-estimates-tra#annual-daily-traffic-flow-and-distribution-tra03" target="_blank" rel="noreferrer">Department for Transport (including table TRA0307)</a>. Routing and Greggs: <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap contributors</a> via OSRM / Photon / Overpass. {plan.liveStatus === 'available' ? 'Live speed: TomTom. ' : ''}<a href="https://nationalhighways.co.uk/roads-and-travel/live-travel-updates/" target="_blank" rel="noreferrer">Check current incidents with National Highways</a>. QuietRoad is independent and is not affiliated with Greggs plc.</footer>
        </>}
      </div>
    </div>
  </main>;
